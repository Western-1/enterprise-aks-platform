...
- **No secrets in git.** The PostgreSQL password is generated by Terraform and written
  ...
  The app itself authenticates to PostgreSQL **passwordless** — with a short-lived
  Microsoft Entra token fetched through its workload identity (see
  [architecture](docs/architecture.md)).
- **Workload Identity.** Pods authenticate to Azure with a federated service account — no client secrets.
  The same identity gets both Key Vault secrets (CSI driver) and PostgreSQL access tokens (Entra ID).
...
![Demo application pods](docs/screenshots/media-pods.png)

*Demo namespace `media`: `media-api` + `media-worker` Running, Redis Running, `db-init` job Completed (created the `media` database with the workload identity).*
